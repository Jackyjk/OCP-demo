...
CREATE TABLE `t_department` (
  ...
  `f_headline` VARCHAR(128) NOT NULL COMMENT '抬头信息',
...
CREATE TABLE `t_warehouse` (
  ...
  `f_update_time` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`f_id`),
  CONSTRAINT `fk_agent_user_warehouse` FOREIGN KEY(`f_agent_user_id`) REFERENCES `t_agent_user`(`f_id`)
)ENGINE=INNODB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `t_order` (
  ...
  `f_money` INT(16) COMMENT '总金额',
  `f_number` INT(4) COMMENT '总数量',
  `f_first_operator_id` INT(10) COMMENT '初审运营人员id',
  `f_second_operator_id` INT(10) COMMENT '复审运营人员id',
  `f_agent_user_id` INT(10) NOT NULL COMMENT '经销商id',
  `f_warehouse_id` INT(10) NOT NULL COMMENT '仓库id',
  `f_shop_id` INT(10) NOT NULL COMMENT '店铺id',
  `f_del_flag` TINYINT(4) UNSIGNED NOT NULL DEFAULT '0' COMMENT '删除标志：0正常，1 删除',
  `f_create_time` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `f_update_time` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`f_id`),
  ...
  CONSTRAINT `fk_order_warehouse` FOREIGN KEY(`f_warehouse_id`) REFERENCES `t_warehouse`(`f_id`),
  CONSTRAINT `fk_order_shop` FOREIGN KEY(`f_shop_id`) REFERENCES `t_shop`(`f_id`)
)ENGINE=INNODB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4;
